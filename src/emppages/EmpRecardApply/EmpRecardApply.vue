<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 用户列表区域 -->
      <el-table :data="recardApplyList" stripe>
        ...
        <el-table-column label="补卡日期" prop="recardDate"></el-table-column>
        <el-table-column label="上班时间" prop="morning"></el-table-column>
        <el-table-column label="下班时间" prop="afternoon"></el-table-column>
  ...
  <el-dialog
    ...
    <div>
      <el-form :model="addForm" ref="addFormRef" label-width="100px">
        ...
        <el-form-item label="补卡日期" prop="recardDate">
          <el-input v-model="addForm.recardDate"></el-input>
        </el-form-item>
  ...
  <el-dialog
    ...
    <el-form :model="editForm" ref="editFormRef" label-width="70px">
      ...
      <el-form-item label="补卡日期" prop="recardDate">
        <el-input v-model="editForm.recardDate"></el-input>
      </el-form-item>
      <el-form-item label="上班时间" prop="morning">
      ...
      <el-form-item label="审批状态" prop="isExam">
...
import {
  api_addRecardApply, api_editRecardApply,
  api_getRecardApply,
  api_getRecardApplyByID,
  api_removeRecardApply
} from "@/utils/api/recard/recard"
...
export default defineComponent({
  ...
  setup() {
    ...
    const getUserList = async () => {
      const {data: res} = await api_getRecardApply(data.queryInfo);
      if (res.meta.code !== 200) {
        ElMessage({type: "error", message: "服务器开小差了", duration: 1800});
      }
      data.recardApplyList = JSON.parse(JSON.stringify(res.data.recardApplyList));
      data.recardApplyList = data.recardApplyList.filter(item => item.employee_id == data.id)
      data.total = res.data.total;
      let promiseList = data.recardApplyList.map(async val => {
    ...
    onBeforeMount(async () => {
      await getUserList();
      console.log("data.recardApplyList", data.recardApplyList)
    });
    //监听 pageSize 改变的事件
    ...
    const addDept = async () => {
      ...
      const {data: res} = await api_addRecardApply(data.addForm);
      if (res.meta.code !== 200) {
        return ElMessage({
          type: "error",
          message: "id重复"
    ...
    const showEditDialog = async (id) => {
      ...
      const {data: res} = await api_getRecardApplyByID(id);
      if (res.meta.code !== 200) {
        ElMessage({
          type: "error",
          message: "请求失败"
    ...
    const removeApplyById = async (id) => {
      ...
      if (confirmResult !== "confirm") {
        ...
        const {data: res} = await api_removeRecardApply(id);
        if (res.meta.code !== 200) {
          return ElMessage({
            type: "error",
            message: "删除失败"
    ...
    const editApply = async () => {
      await api_editRecardApply(data.editForm)
      data.editDialogVisible = false
      await getUserList()
    ...
    const filterByName = async (name) => {
      await getUserList()
      let filterList = data.recardApplyList.filter(item => item.isExam.indexOf(name) >= 0)
      data.recardApplyList = filterList
    }
    //添加申请
    const addDialogFun = () => {
