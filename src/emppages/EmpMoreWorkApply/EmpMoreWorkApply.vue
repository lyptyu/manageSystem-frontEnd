<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 用户列表区域 -->
      <el-table :data="moreWorkApplyList" stripe>
        ...
        <el-table-column label="加班日期" prop="moreWorkDate"></el-table-column>
        <el-table-column label="开始时间" prop="startTime"></el-table-column>
        <el-table-column label="结束时间" prop="endTime"></el-table-column>
        <el-table-column label="加班时长" prop="moreWorkHours"></el-table-column>
        <el-table-column label="加班津贴" prop="extraMoney"></el-table-column>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getUserList = async () => {
      const {data: res} = await api_getMoreWorkApply(data.queryInfo);
    ...
    onBeforeMount(async () => {
      await getUserList();
      console.log("data.moreWorkApplyList", data.moreWorkApplyList)
    });
    //监听 pageSize 改变的事件
    ...
    const showEditDialog = async (id) => {
      ...
      const {data: res} = await api_getMoreWorkApplyByID(id);
      if (res.meta.code !== 200) {
        ElMessage({
          type: "error",
          message: "请求失败"
